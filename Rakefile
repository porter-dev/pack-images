...
task "fetch:latest" do
  ...
  builder_files.each do |path|
    ...
    toml_hash["buildpacks"].map! do |hash|
      id = hash["id"]
      hash["uri"] = fetcher_for_id[id].latest_entry.addr
      hash
    end

    toml_hash["order"].map! do |hash|
      id = hash["id"]
      hash["version"] = fetcher_for_id[id].latest_entry.version
      hash
    end

    puts toml_hash
    # path.write(TomlRB.dumper(toml_hash))
  end
end
